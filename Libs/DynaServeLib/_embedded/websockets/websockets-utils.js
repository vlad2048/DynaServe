...
export function getReqScriptsSyncMsg() {
    return {
...
export function handleReplyScriptsSync(cssLinksDel, cssLinksAdd, jsLinksDel, jsLinksAdd) {
    ...
    if (!isFirstTime)
        LinkUtils.getCssScripts().forEach((e) => (e.href = LinkUtils.bumpLnk(e.href)));
...
export function handleScriptRefresh(type, link) {
    switch (type) {
        case "Css": {
            const nodes = LinkUtils.getCssScripts().filter((e) => LinkUtils.isMatch(e.href, link));
            if (nodes.length === 0)
